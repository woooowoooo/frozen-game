...
const LOCK_SPEED = 40;
export const MOVE_TEXT_DURATION = 60;
// State variables
...
class Piece {
	constructor (type, center, rotation, isGhost = false) {
		...
		if (!isGhost) {
			if (game.queue.length < NEXT_AMOUNT) {
				game.queue.push(...newBag());
			}
			updateGhost(this);
			ghost.fill();
		}
...
function collisionCheck(piece, cell) {
	return game.cells[cell] !== " " && game.cells[cell] !== "_" && !piece.blocks.includes(cell) && cell >= 0;
}
// New game
function newBag() {
...
export function newGame(newMode = game.mode) {
	heldKeys.clear();
	game.mode = newMode;
	game.cells = Array(CELL_AMOUNT).fill(" ");
	game.queue = newBag();
	game.held = null;
	current = new Piece(game.queue.shift());
	moveText = null;
	endGameText = null;
	hasHeld = false;
	game.changed = true;
...
function endGame(win) {
	...
	highScores[game.mode] = game.mode === "fortyLines" ? Math.min(time, highScores[game.mode] ?? Infinity) : Math.max(score, highScores[game.mode] ?? 0);
	const endText = {
		default: [`Score: ${score}`, `High Score: ${highScores[game.mode]}`],
		fortyLines: [`Time: ${time / 1000} seconds`, `Fastest Time: ${highScores[game.mode] / 1000} seconds`]
	};
	endGameText = endText[game.mode];
}
// Game mechanics
...
function scoreMove(linesCleared) {
	if (tSpin) {
		...
		const filled = corners.map(cell => game.cells[cell] !== " ");
...
function lock() { // Returns whether the game is over
	...
	if (lines.has(1)) {
		endGame(game.mode === "default");
		return;
	}
	const fullLines = [...lines].filter(line => game.cells.slice(line * COLS + 1, (line + 1) * COLS).every(cell => cell !== " "));
	...
	for (const line of fullLines) {
		game.cells.splice(line * COLS, COLS);
		game.cells.unshift(...Array(COLS).fill(" "));
	}
	// Perfect clear bonus
	...
	if (game.mode === "fortyLines" && totalLines >= 40) {
		endGame(true);
		return;
	}
	// New piece
...
export function handle({key, location}) {
	if (key === "Escape") {
		...
		current.clear();
		ghost.clear();
		[game.held, current] = [current.type, new Piece(game.held ?? game.queue.shift())];
	}
}
export function update() {
	...
	if (moveTextTimer > 0) {
		game.changed = true; // Text color changes
		moveTextTimer--;
	} else if (moveText != null) {
		moveText = null;
	}
	return [game.mode === "fortyLines" ? true : game.changed, endGameText];
}
export function getInfo() {
	if (game.mode === "default") {
		return [{
			Score: score,
			HScore: (highScores[game.mode] ?? "None"),
			Lines: totalLines,
			Combo: combo,
			B2B: hardMove
		}, {moveText, moveTextTimer}];
	} else { // Forty lines
		return [{
			Time: (time / 1000).toFixed(3) + "s",
			FTime: (highScores[game.mode] != null) ? ((highScores[game.mode] / 1000).toFixed(3) + "s") : "None",
			Lines: `${totalLines} / 40`,
			Combo: combo,
			B2B: hardMove
		}, {moveText, moveTextTimer}];
